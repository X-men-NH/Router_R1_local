...
def get_client(
    ...
    if _cached_client is None:
        max_retries = int(os.getenv('ROUTER_API_MAX_RETRIES', str(max_retries)))
        timeout = float(os.getenv('ROUTER_API_TIMEOUT', str(timeout)))
...
def get_llm_response_via_api(prompt,
    ...
    '''
    max_trials = int(os.getenv('ROUTER_API_MAX_TRIALS', str(MAX_TRIALS)))
    time_gap = float(os.getenv('ROUTER_API_RETRY_GAP_SEC', str(TIME_GAP)))
    ...
    while max_trials:
        ...
        try:
            ...
            if "request timed out" in err or "timed out" in err:
                break
            if "invalid api key" in err or "authentication" in err or "unauthorized" in err:
                break
            print("Retrying...")
            time.sleep(time_gap)

    if completion is None:
        raise Exception("Reach MAX_TRIALS={}".format(int(os.getenv('ROUTER_API_MAX_TRIALS', str(MAX_TRIALS)))))
...
API_PRICE_1M_TOKENS = {
    "qwen/qwen-2.5-7b-instruct": 0.3,
    "meta-llama/llama-3.1-70b-instruct": 0.88,
    "meta-llama/llama-3.1-8b-instruct": 0.18,
    "mistralai/mistral-nemo": 0.2,
    "mistralai/mixtral-8x22b-instruct": 1.2,
    "google/gemma-2-27b-it": 0.8,
    "writer/palmyra-creative-122b": 1.8,
    "nvidia/llama3-chatqa-1.5-8b": 0.18,
    "nvidia/llama-3.1-nemotron-51b-instruct": 1.0,
    "nvidia/llama-3.3-nemotron-super-49b-v1": 1.0,
...
def request_task(data):
    ...
    if LLM_NAME == "":
        print(f"[WARNING] LLM Name Error: q_id={q_id}, query_text={query_text[:80]!r}")
    ...
    price = API_PRICE_1M_TOKENS.get(LLM_NAME, 0.0)
...
def check_llm_name(target_llm):
    ...
    if "qwen" in target_llm:
        LLM_NAME = "qwen/qwen-2.5-7b-instruct"
        ...
        if "70b" in target_llm:
            ...
            print(f"[WARNING] Unrecognized llama variant: {target_llm!r}")
            LLM_NAME = "meta-llama/llama-3.1-8b-instruct"
        ...
        print(f"[WARNING] Unrecognized LLM name: {target_llm!r}")
        LLM_NAME = ""
...
def access_routing_pool(queries, api_base, api_key):
    ...
    for q_id, single_query in enumerate(queries):
        ...
        if len(parts) < 2:
            print(f"[WARNING] Malformed query (no ':' separator), q_id={q_id}: {single_query[:80]!r}")
            task_args.append((q_id, single_query, 0, "", api_base, api_key))
            continue
